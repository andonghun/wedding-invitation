...
const LandingPage = () => {
  ...
  const spin = () => {
    ...
    const isWin = spinCount >= 3 && Math.random() < 0.5;

    let spinningDoors;

    if (isWin) {
      const windIndex = Math.floor(Math.random() * WinResultList.length);
      spinningDoors = WinResultList[windIndex];
    } else {
      // 회전 시작
